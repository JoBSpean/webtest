-- Migration version matches the applied Supabase migration history.
...
create schema karting_private;
revoke all on schema karting_private from public, anon, authenticated;
grant usage on schema karting_private to authenticated;

create function karting_private.submit_lap(p_track text, p_cls text, p_name text, p_time_ms integer)
...
$$;
revoke all on function karting_private.submit_lap(text, text, text, integer) from public, anon;
grant execute on function karting_private.submit_lap(text, text, text, integer) to authenticated;

-- Only the unprivileged wrapper is exposed through the Data API.
create function public.submit_karting_lap(p_track text, p_cls text, p_name text, p_time_ms integer)
returns void language sql security invoker set search_path = '' as $$
  select karting_private.submit_lap(p_track, p_cls, p_name, p_time_ms);
$$;
